...
例如，这里有一个 ``，具有 `contentEditable` 属性。该属性用于聚焦和编辑节点。
...
想象一下，当您在页面上附加了一个第三方脚本，想在页面上添加一个有用的功能，但也同时增添了一些不必要的功能，比如，显示了广告 `Unwanted ads`。
...
到目前为止，一切都很简单，对吧？HTML 中有 `` 代码片段，并高亮显示了。

现在继续。假设我们要从服务器动态获取资料。当前我们只关心从网络服务器获取 HTML 文章后按需显示：
