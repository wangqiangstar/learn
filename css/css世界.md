...
- 1. 收缩和包裹。浮动、绝对定位、inline-block元素或table元素，宽度收缩到合适，CSS3中叫做fit-content。
- 1. 收缩到最小。在规范中被描述为min-content。例如table-layout为auto的表格。
...
- 1. 超出容器限制。以上3种情况尺寸都不会主动超过父级容器宽度，但是存在特殊情况，例如内容很长的连续的英文和数字，或者内联元素被设置了white-space:norwap(文本不会换行，文本会在在同一行上继续，直到遇到
     标签为止)。
...
- 如果上述条件都不符合，则最终宽度表现为300px，高度变现为150px。例如（chrome下是0x0）、。
...
既然图片是固有尺寸，为什么修改img的width和height能修改图片的大小，可以查看 http://www.zhangxinxu.com/wordpress/2015/03/css3-object-position-object-fit/。
...
> 在chrome下此时和“![img](https://camo.githubusercontent.com/e17e4305c1ef2d75c77492e6ffa33424dd99a531784a2e6c4fa70900374c12e7/68747470733a2f2f7777772e62616964752e636f6d2f696d672f62645f6c6f676f312e706e67)”一样。
...
- 父元素设置border-top值
- 父元素设置padding-top值
- 父元素和第一个子元素之间添加内联元素进行分隔 对于margin-bottom满足以下任意条件即可
- 父元素设置为块状格式化上下文元素
- 父元素设置border-bottom值
...
行距 = 行高 - em-box (行距 = line-height - font-size) 半行距 = 行距 / 2
...
1. vertical-align:middle与近似垂直居中
...
浮动的本质是为了实现文字环绕效果。 浮动没有内联元素的间隙问题和margin合并问题。
...
- 父级元素高度塌陷(非BUG), 查看[清除和去除浮动的方法详解](https://ziyi2.github.io/2017/08/02/清除和去除浮动的方法详解.html#more)
- 行框盒子的区域限制
...
> 父元素box此时高度塌陷，详情可查看[清除和去除浮动的方法详解](https://ziyi2.github.io/2017/08/02/清除和去除浮动的方法详解.html#more), 但是文字围绕在图片周围，这就是浮动的作用。
...
1. 块状化 position一旦置为absolute或fixed,其display值就是block或table。
...
如果该祖先元素是inline元素： (1). 假设给内联元素的前后各生成一个宽度为0的内联盒子，则这两个内联盒子的padding box外面的包围盒就是内联元素的“包含块”。 (2). 如果该内联元素被跨行分隔了，那么“包含块”未定义。 否则该祖先元素不是inline元素，“包含块”由该祖先元素的padding box边界形成。 如果没有符合的祖先元素，则“包含块”是“初始包含块”。
...
- 1. float浮动盒子
- 1. inline水平盒子
- 1. z-index:auto或者z-index:0，这里新增不依赖z-index的层叠上下文
- 1. 正z-index
